...
| 4 | ✅ **Cartão via FATURA HOSPEDADA Asaas** (ENTREGUE 2026-06-18; PO escolheu hosted sobre form-próprio SAQ-D): `method='card'` cria subscription `CREDIT_CARD` **sem PAN** (PCI-safe/LGPD/D5) → cliente paga na página segura do Asaas, que tokeniza e auto-cobra os próximos ciclos. Nasce `pending`; módulos só no `PAYMENT_CONFIRMED` (webhook já cobre). Dummy de cartão **removido**. tsc limpo. | 3 | ~½ dia | baixo |
| 5 | ✅ **R2 anual-cartão** (ENTREGUE 2026-06-18, junto do #4): `chargeValueFor()` cobra `12×preço×0,90` (10%) no cartão anual vs `×0,80` (20%) PIX; `cycle=YEARLY` = cobrança única (D1, sem parcelamento). `pricing.ts` ganhou `yearlyDiscountedCard` + `CARD_ANNUAL_DISCOUNT_PERCENT=10`; 2 testes novos (8/8). | 4 | trivial | baixo |
| 6 | ✅ **R7 dunning + cron** (ENTREGUE 2026-06-18): migration **0398** (`past_due_since`); webhook `OVERDUE→past_due`+âncora e `CHARGEBACK/REFUNDED→suspende` (D3); cron diário `/api/cron/subscription-dunning` (03:30, `Bearer CRON_SECRET`) com `planDunningTransition` pura (11 testes); `attemptSuspendSubscription` adia se há internação/prontuário em atendimento (D3); sync rebaixa a Free só em suspended/expired (contratos preservados); banner M1–M4 no topo da dashboard (só admin não-sysmax não-grandfathered). Ignora is_grandfathered (D5). tsc 0 erros. ⚠️ exige `CRON_SECRET` na Vercel. | 3 | ~2 dias | médio |
| 7 | ⏸️ **NFS-e no PAYMENT_CONFIRMED** + cancelamento no estorno — **BLOQUEADO (PO 2026-06-18): aguardando credenciais fiscais da Sysmax como EMISSORA.** Código é reusável (cliente Focus NFe `nfse-focus.ts`/`emitNfse`), mas a Sysmax ainda NÃO existe como prestador no sistema: falta **conta Focus NFe da Sysmax + CNPJ com inscrição municipal habilitada p/ NFS-e + código de serviço LC 116 (~1.05 licenciamento de software) definido com o contador**. A construir quando os dados chegarem: (a) config fiscal da Sysmax (DB recomendado — linha em clinic_fiscal_config marcada como emissora SaaS, ou env vars); (b) migration colunas `nfse_*` em `subscription_invoices`; (c) ação `emitNfseForSubscriptionPayment()` (prestador=Sysmax, tomador=clínica, base ISS líquida do desconto 10/20%); (d) gancho no webhook após activatePaidSubscription + cancelamento no REVERSAL (TODO já marcado em route.ts). Recebimento NÃO depende disso (opera sem NFS-e). | 3 | ~1 dia (após creds) | médio |
