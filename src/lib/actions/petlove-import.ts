...
async function applyPreviewSideEffects(
  ...
  for (const line of lines) {
    ...
    const serviceDate = line.service_date as string
    // Calendário oficial Petlove: paga até o dia 30 do mês seguinte ao
    // procedimento. Ex.: atendimento 15/05 → vencimento 30/06.
    const dueDate = computePetloveDueDate(serviceDate)

    const description = `Petlove (em aberto) · ${procName} · ${petName} (${tutorName}) · atendido ${fmtBR(serviceDate)}`
...
// computePetloveDueDate movido para @/lib/utils/petlove-dates
