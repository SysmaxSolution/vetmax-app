...
/**
 * Nível 3 da pipeline de hidratação do Canvas Visual — extração de
 * entidades estruturadas a partir de texto livre (anamnese, notas do
 * MV, transcrição de áudio).
 *
 * O motor de Repeater (`RepeaterRenderer`) consome arrays com forma
 * canônica de `MOCK_PRESCRIPTIONS`. Quando o MV não cadastra item-a-item
 * na tabela `prescriptions` (caso comum em consultas rápidas — ele
 * ditou "vou receitar Dipirona 20mg de 8 em 8 horas por 5 dias") os
 * blocos de receita aparecem vazios. Esta server action lê o texto
 * livre e devolve a lista estruturada que o repeater consegue
 * renderizar com agrupamento, numeração e destaques de controlados.
 *
 * Para evitar chamadas custosas, o caller deve invocar APENAS quando
 * a tabela `prescriptions` da consulta estiver vazia. Em consultas
 * com prescrições formais, esses dados têm precedência.
 */
...
/** Trunca para o limite do prompt sem cortar no meio de uma sentença
 *  (procura o último ponto até 200 chars antes do limite). */
...
/** Heurística para evitar chamada de IA quando o texto claramente
 *  não fala de prescrição. */
...
/**
 * Extrai prescrições de texto livre. Tenta IA quando há indício de
 * prescrição; sempre faz fallback para o parser determinístico se a
 * IA falhar ou retornar vazio.
 *
 * Não levanta — em qualquer falha devolve `{ prescriptions: [], ... }`
 * para que o caller siga renderizando o repeater vazio com segurança.
 */
export async function extractEntitiesFromAnamnese(
  text: string | null | undefined,
): Promise<ExtractedAnamneseEntities> {
  // Autenticação leve — só para garantir que esta server action não vire
  // endpoint público de IA (custo $$$).
  ...
  // Heurística defensiva: parser determinístico primeiro. Se ele já
  // produzir resultados (ex: vet colou uma receita linha-a-linha bem
  // formatada) usamos esses dados e nem chamamos IA.
  const parsed = parseMedicamentosText(raw)
  if (parsed.length > 0) {
    return { prescriptions: parsed, confidence: 'high', source: 'parser' }
  }

  // Sem chave de IA ou sem indício de prescrição → encerra cedo.
  ...
  try {
    ...
    console.error('[extractEntitiesFromAnamnese] failed:', e)
    return { prescriptions: [], confidence: 'low', source: 'ai' }
  }
}

/** Normaliza um objeto retornado pela IA para a forma `ParsedPrescription`,
 *  cobrindo variações de nome de campo e enums. */
