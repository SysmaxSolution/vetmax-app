...
export async function reversePartialPayment(
  ...
  // 4) Reconciliação DEFENSIVA do saldo a receber (source='cashier').
  // Antes usávamos .maybeSingle(): se já existisse MAIS DE UM pending cashier,
  // ele retornava erro/null e o estorno INSERIA outro → "Saldo a receber"
  // duplicado. Agora garantimos EXATAMENTE UM pending com o saldo correto
  // (newBalance, idempotente), consolidando/removendo eventuais duplicados.
  // Não toca em card_acquirer/petlove_open — esses são recebíveis de terceiros
  // (operadora/convênio), não saldo do tutor.
  const { data: cashierPendings } = await adminClient
    ...
    .eq('source', 'cashier')
    .order('created_at', { ascending: true })
  const pendings = cashierPendings ?? []

  if (newBalance > 0.01) {
    if (pendings.length > 0) {
      // mantém o primeiro com o saldo correto; remove duplicados órfãos
      await adminClient
        .from('financial_entries')
    ...
    // saldo zerado: remove todos os pending cashier órfãos
