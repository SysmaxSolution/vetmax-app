...
import { Scissors, Clock, CheckCircle2, Loader2, X, Calendar, DollarSign, CheckCheck } from 'lucide-react'
...
import { finishGroomingSessionAndRecord } from '@/lib/actions/grooming-cashier'
...
export default function GroomingKanban({ initialBoard, clinicId }: Props) {
  ...
  const confirmDeliver = async () => {
    ...
    // 1. Registrar pagamento no Caixa Central via RPC ANTES de mover para 'delivered'
    //    O RPC exige current_status = 'waiting_pickup' — deve ser chamado enquanto
    //    o card ainda está nessa coluna.
    if (card.price_total && card.price_total > 0) {
      const payResult = await finishGroomingSessionAndRecord(
        card.id,
        `Banho e Tosa — ${card.patient.name} (${card.services_requested.join(', ')})`
      )
      if ('error' in payResult) {
        // Falha silenciosa: loga mas não bloqueia a entrega
        console.warn('[Grooming] Falha ao registrar no caixa:', payResult.error)
      }
    }

    // 2. Atualizar status para 'delivered' no Kanban
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">
                  {pendingDeliver.patient.name} está pronto para retirada?
                </p>
              </div>
              <button
...
function GroomingKanbanCard({ card, isScheduled, isConfirmingArrival, onDragStart, onOpen, onDeliver, onConfirmArrival, onCancel }: CardProps) {
