...
import { Loader2, Upload, CheckCircle2, AlertTriangle, HelpCircle, Link2 } from 'lucide-react'
import {
  parseCardStatement, matchCardStatement, reconcileCardInstallments,
      ...
      {result && (
        <>
          ...
          {groups.map(g => {
            ...
            const selectable = g.key === 'linked' || g.key === 'divergent'
            return (
                      ...
                      {rows.map((r, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-slate-50/50">
                            {selectable && <td className="px-3 py-2"><input type="checkbox" checked={r.installment_id ? selected.has(r.installment_id) : false} onChange={() => r.installment_id && toggle(r.installment_id)} /></td>}
          ...
          <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3">
            <p className="text-xs text-slate-500">{selected.size} selecionado(s) para conciliar. Divergentes selecionados serão atualizados com os dados do extrato.</p>
