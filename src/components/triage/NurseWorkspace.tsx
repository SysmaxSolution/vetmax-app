...
const STATUS_BADGE: Record<string, string> = {
  ...
  waiting_exam: 'bg-indigo-100 text-indigo-700',
      ...
      <main className="mx-auto max-w-4xl px-3 sm:px-6 py-6 sm:py-8 space-y-6 animate-enter">
        ...
        <div className="flex gap-1 rounded-xl bg-slate-100 p-1 w-fit">
          ...
          >
            ...
            Fila de Espera
            {localQueue.length > 0 && (
              <span className={`ml-1 rounded-full px-2 py-0.5 text-xs font-bold font-mono tabular-nums ${
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-200 px-6 py-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-600 font-mono tabular-nums">
        ...
        {tab === 'historico' && (
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-200 px-6 py-4 flex items-center justify-between">
              ...
              <span className="rounded-full bg-teal-100 px-3 py-1 text-sm font-semibold text-teal-700 font-mono tabular-nums">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.vital_signs && (
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-xs font-semibold font-mono tabular-nums bg-sky-100 text-sky-700 px-2 py-0.5 rounded-full">
                            {item.vital_signs.weight} kg
                          </span>
                          <span className="text-xs font-mono tabular-nums bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 space-y-4 animate-scale-in">
            ...
            <div>
              ...
              {selectedPatient ? (
                <div className="flex items-center justify-between rounded-lg border border-teal-200 bg-teal-50 px-3 py-2">
                  <span className="text-sm font-medium text-teal-900">{selectedPatient.name} — {selectedPatient.tutor.name}</span>
                  <button type="button" onClick={() => { setSelectedPatient(null); setPatientSearch(''); setPatientResults([]) }} className="text-xs text-teal-700 hover:underline ml-2">Trocar</button>
                </div>
              ) : (
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
                  />
                  {patientResults.length > 0 && (
                    <div className="absolute z-10 w-full bg-white border border-slate-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto animate-scale-in origin-top">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-4 py-2 border border-slate-200 bg-white rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-semibold shadow-sm hover:bg-teal-700 disabled:opacity-60 transition-colors"
              >
