...
export default function BillingWorkspace({
  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-7xl mx-auto animate-enter">
      {toast && (
        <div className={`fixed top-4 right-4 z-[100] rounded-xl px-4 py-2.5 text-sm font-semibold shadow-lg animate-enter-fast ${toast.type === 'success' ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'}`}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900">Faturamento</h1>
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg bg-teal-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-teal-700 focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2 transition-colors"
        >
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          ...
          <Kpi icon={<Clock className="h-4 w-4" />} label="Orçam. em aberto" value={String(summary.open_count)} sub="aguardando" color="text-sky-700 bg-sky-50" />
          <Kpi icon={<FileText className="h-4 w-4" />} label="Valor total" value={fmt(summary.total)} sub="no período" color="text-emerald-700 bg-emerald-50" />
        </div>
      )}
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-4 mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div className="flex items-end">
            <button onClick={applyFilters} disabled={loading} className="w-full flex items-center justify-center gap-1.5 rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700 disabled:opacity-50">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {docs.map(d => (
                    <tr key={d.id} onClick={() => setActiveId(d.id)} className="hover:bg-teal-50/40 cursor-pointer transition-colors">
                      <td className="px-4 py-3 font-mono text-xs font-semibold text-slate-800">{d.doc_number}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${d.doc_type === 'nfse' ? 'bg-indigo-100 text-indigo-700' : 'bg-emerald-100 text-emerald-700'}`}>
                          {d.doc_type === 'nfse' ? 'NFS-e' : 'Orçamento'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-600 font-mono tabular-nums">{fmtDate(d.issue_date)}</td>
                      <td className="px-4 py-3 text-slate-600 font-mono tabular-nums">{fmtDate(d.billed_date)}</td>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-slate-900 font-mono tabular-nums">{fmt(d.total_amount)}</td>
            ...
            {docs.map(d => (
              <button key={d.id} onClick={() => setActiveId(d.id)} className="w-full text-left rounded-xl border border-slate-200 bg-white px-4 py-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${d.doc_type === 'nfse' ? 'bg-indigo-100 text-indigo-700' : 'bg-emerald-100 text-emerald-700'}`}>{d.doc_type === 'nfse' ? 'NFS-e' : 'Orçamento'}</span>
                </div>
                <p className="mt-1 text-sm font-medium text-slate-800 truncate">{d.patient_name ?? d.tutor_name ?? '—'}</p>
                <div className="mt-1 flex items-center justify-between">
                  <span className="text-xs text-slate-400 font-mono tabular-nums">{fmtDate(d.issue_date)}</span>
                  <span className="text-sm font-bold text-slate-900 font-mono tabular-nums">{fmt(d.total_amount)}</span>
                </div>
              </button>
            ))}
          </div>
...
const inputCls = 'w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500'
...
function Kpi({ icon, label, value, sub, color }: { icon: React.ReactNode; label: string; value: string; sub: string; color: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-4">
      ...
      <p className="text-lg font-bold text-slate-900 font-mono tabular-nums">{value}</p>
