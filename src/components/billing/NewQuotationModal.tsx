      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl my-4 flex flex-col max-h-[92vh] animate-scale-in">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-teal-50/40 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-teal-600"><FileText className="h-5 w-5 text-white" /></div>
            <div>
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="relative" ref={tutorWrapRef}>
              ...
              {tutor ? (
                <div className="flex items-center gap-2 bg-teal-50 border border-teal-200 rounded-xl px-3 py-2">
                  <span className="flex-1 text-sm font-semibold text-teal-800 truncate">{tutor.name}</span>
                  <button onClick={() => { setTutor(null); setPetOptions([]); setPetId(''); setTutorQuery('') }} className="text-teal-500 hover:text-teal-700"><X className="h-4 w-4" /></button>
                </div>
              ) : (
                <>
                  <div className="relative">
                    <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
                    <input value={tutorQuery} onChange={e => handleTutorQuery(e.target.value)} placeholder="CPF, nome do tutor ou pet" className="w-full pl-8 pr-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
                  </div>
                  {tutorResults.length > 0 && (
                    <div className="absolute z-20 mt-1 w-full rounded-xl border border-slate-200 bg-white shadow-lg max-h-52 overflow-y-auto">
                      {tutorResults.map(r => (
                        <button key={r.tutor.id} onClick={() => pickTutor(r)} className="w-full text-left px-3 py-2 hover:bg-teal-50 border-b border-slate-100 last:border-0">
            ...
            <div>
              ...
              <select value={petId} onChange={e => setPetId(e.target.value)} disabled={petOptions.length === 0} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm disabled:bg-slate-50 disabled:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select value={professionalId} onChange={e => setProfessionalId(e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20">
            ...
            <div>
              ...
              <input type="date" value={validUntil} onChange={e => setValidUntil(e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            </div>
          </div>
          ...
          <div className="relative" ref={svcWrapRef}>
            ...
            <div className="relative">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
              <input value={svcQuery} onChange={e => handleSvcQuery(e.target.value)} onFocus={() => svcQuery.length < 2 && handleSvcQuery(svcQuery)} placeholder="Buscar serviço no catálogo..." className="w-full pl-8 pr-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            </div>
            {(svcSearching || svcResults.length > 0) && (
              <div className="absolute z-20 mt-1 w-full rounded-xl border border-slate-200 bg-white shadow-lg max-h-52 overflow-y-auto">
                {svcSearching ? (
                  ...
                  <button key={s.id} onClick={() => addService(s)} className="w-full flex items-center justify-between px-3 py-2 hover:bg-teal-50 border-b border-slate-100 last:border-0">
            ...
            <button onClick={addManualLine} className="mt-1.5 inline-flex items-center gap-1 text-xs font-semibold text-slate-500 hover:text-teal-700">
            ...
            <div className="rounded-xl border border-slate-200 divide-y divide-slate-100">
              {lines.map(l => (
                <div key={l.key} className="flex flex-wrap items-center gap-2 px-3 py-2">
                  <input value={l.description} onChange={e => updateLine(l.key, { description: e.target.value })} placeholder="Descrição" className="flex-1 min-w-[120px] basis-40 rounded-lg border border-slate-200 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-teal-500/30" />
                  <div className="flex items-center gap-1">
                    ...
                    <input type="number" min="0" step="0.001" value={l.quantity} onChange={e => updateLine(l.key, { quantity: parseFloat(e.target.value) || 0 })} className="w-12 text-center rounded-lg border border-slate-200 px-1 py-1 text-sm font-mono tabular-nums" />
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <input type="number" min="0" step="0.01" value={l.unit_price === 0 ? '' : l.unit_price} placeholder="0,00"
                      ...
                      className="w-20 text-right rounded-lg border border-slate-200 px-2 py-1 text-sm font-mono tabular-nums disabled:bg-slate-100 disabled:text-slate-400" />
                  </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            <input value={paymentMethods} onChange={e => setPaymentMethods(e.target.value)} placeholder="Formas de pagamento (ex.: PIX, cartão 3x...)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            <input value={discountNote} onChange={e => setDiscountNote(e.target.value)} placeholder="Descontos (opcional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            <textarea value={observations} onChange={e => setObservations(e.target.value)} rows={2} placeholder="Observações (opcional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
          </div>
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-4 border-t border-slate-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xl font-bold text-slate-900 font-mono tabular-nums">{fmt(total)}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={onClose} disabled={saving} className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50">Cancelar</button>
            <button onClick={handleSave} disabled={saving || lines.length === 0} className="rounded-lg bg-teal-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-teal-700 disabled:opacity-50 flex items-center gap-2">
