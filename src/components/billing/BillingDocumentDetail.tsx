      ...
      <div className="w-full max-w-lg bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-green-100"><FileText className="h-5 w-5 text-green-600" /></div>
            <div>
        ...
        {loading || !doc ? (
          <div className="flex-1 flex items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-green-600" /></div>
        ) : (
          <>
                  ...
                  {doc.items.map((it, i) => (
                    <div key={it.id ?? i} className="flex items-center justify-between px-4 py-2 text-sm">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-slate-400">{Number(it.quantity).toLocaleString('pt-BR')} × {fmt(it.unit_price)}</p>
                      </div>
                      <span className="font-semibold text-slate-900 tabular-nums ml-3">{fmt(it.total_price)}</span>
                    </div>
                  ))}
                  <div className="flex items-center justify-between px-4 py-3 bg-slate-900">
                    <span className="text-xs font-semibold text-slate-300 uppercase tracking-wide">Total</span>
                    <span className="text-lg font-bold text-white tabular-nums">{fmt(doc.total_amount)}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-2 px-5 py-4 border-t border-slate-100 flex-shrink-0">
              <button onClick={handlePdf} disabled={!!busy} className="flex items-center gap-1.5 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-700 disabled:opacity-50">
                {busy === 'pdf' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />} PDF
              </button>
              {doc.tutor_id && (
                <button onClick={handleWhatsApp} disabled={!!busy} className="flex items-center gap-1.5 rounded-xl bg-green-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-green-700 disabled:opacity-50">
              ...
              {isQuote && canEditState && doc.status === 'draft' && (
                <button onClick={handleMarkSent} disabled={!!busy} className="flex items-center gap-1.5 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-blue-700 hover:bg-blue-100 disabled:opacity-50">
                  {busy === 'sent' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Marcar enviado
                </button>
              )}
              {canEditState && (
                <button onClick={handleCancel} disabled={!!busy} className="flex items-center gap-1.5 rounded-xl border border-rose-200 px-4 py-2.5 text-sm font-semibold text-rose-600 hover:bg-rose-50 disabled:opacity-50 ml-auto">
