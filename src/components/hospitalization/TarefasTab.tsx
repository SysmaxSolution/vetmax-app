...
import { Loader2, Plus, Check, CircleStop, FlaskConical, Stethoscope, Utensils, ClipboardList } from 'lucide-react'
import {
...
export default function TarefasTab({ hospitalizationId, onChanged }: Props) {
  ...
  async function handleCreate() {
    ...
    setBusy(true)
    const res = await createHospitalizationTask({ hospitalization_id: hospitalizationId, kind, description: desc.trim(), frequency_hours: freq })
    setBusy(false)
    if ('error' in res) { setError(res.error); return }
    setDesc(''); await reload(); onChanged?.()
  }
    ...
    <div className="flex-1 overflow-y-auto p-5 space-y-4" data-testid="tarefas-tab">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleCreate} disabled={busy} data-testid="task-create-btn"
            ...
            {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Plus className="h-3.5 w-3.5" />} Agendar
          </button>
        </div>
